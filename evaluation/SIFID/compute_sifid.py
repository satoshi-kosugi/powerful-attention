...
parser.add_argument(
    "--device", type=str, default="cuda:0", help="Device to use. Like cuda, cuda:0 or cpu"
)
parser.add_argument(
    "--dims",
    type=int,
...
def calculate_fid_given_paths(paths, batch_size, device, dims, num_workers=1):
    ...
    files0 = sorted(
        [file for ext in IMAGE_EXTENSIONS for file in path0.glob("*.{}".format(ext))]
    )
    ...
    for i in tqdm(range(len(files0))):
        m1, s1 = compute_statistics_of_path(
            [files0[i]], model, batch_size, dims, device, num_workers
        )
        m2, s2 = compute_statistics_of_path(
            [files1[i]], model, batch_size, dims, device, num_workers
        )
        fid_value = calculate_frechet_distance(m1, s1, m2, s2)
        fid_values.append(fid_value)

    return np.array(fid_values).mean()
