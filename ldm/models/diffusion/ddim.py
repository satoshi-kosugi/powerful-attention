...
class DDIMSampler(object):
    ...
    def ddim_sampling(self, cond, shape, negative_conditioning=None,
                      ...
                      style_img=None,style_guidance=1.,content_guidance=1.,start_step=9999,end_step=-9999,disable_tqdm=False,enable_grad=False):
        with (torch.enable_grad() if enable_grad else torch.no_grad()):
            ...
            for i, step in enumerate(iterator):
                ...
                if index >= start_step or index < end_step:
                    continue
    ...
    def p_sample_ddim(self, x, c, t, index, negative_conditioning=None,
        ...
        with (torch.enable_grad() if enable_grad else torch.no_grad()):
            ...
            self.sqrt_one_minus_at = sqrt_one_minus_at
            self.a_t = a_t
            self.a_prev = a_prev
            ...
            self.coef3 = (1. - a_prev - sigma_t**2).sqrt()
            # x_prev = a_prev.sqrt() * pred_x0 + dir_xt + noise
            # import ipdb; ipdb.set_trace()
            self.coef1 = (a_prev / a_t).sqrt()
            self.coef2 = (a_prev).sqrt() * ((1 / a_prev - 1).sqrt() - (1 / a_t - 1).sqrt())
            x_prev = self.coef1 * x + self.coef2 * e_t + noise
